...
static void register_hooks()
{
	ntcreatefile_handle = hookapi_hook_function_byname(
			"ntdll.dll", "NtCreateFile", 1, targetcr3, NtCreateFile_call, NULL, 0);

	//blockbegin_handle = DECAF_registerOptimizedBlockBeginCallback(hook_all,
	//			NULL, 0, OCB_ALL);
}


static void loadmainmodule_callback(procmod_Callback_Params* params)
{
    if(targetcr3 != 0) //if we have found the process, return immediately
    	return;

	if (strcasecmp(targetname, params->lmm.name) == 0) {
		targetpid = params->lmm.pid;
		targetcr3 = params->lmm.cr3;
		register_hooks();
...
static void removeproc_callback(procmod_Callback_Params* params)
{
...
static int hookapitests_init(void)
{
	...
	processbegin_handle = procmod_register_callback(PROCMOD_PROCESSBEGIN_CB,
			&loadmainmodule_callback, NULL);
	removeproc_handle = procmod_register_callback(PROCMOD_REMOVEPROC_CB,
...
static void hookapitests_cleanup(void)
{
	...
	if (processbegin_handle != DECAF_NULL_HANDLE) {
		procmod_unregister_callback(PROCMOD_PROCESSBEGIN_CB,
				processbegin_handle);
