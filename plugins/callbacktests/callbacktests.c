...
static void callbacktests_loadmainmodule_callback(procmod_Callback_Params* params)
{
  char procname[64];
  uint32_t pid;
  ...
  find_process(params->lmm.cr3, procname, 64, &pid);
...
static void callbacktests_removeproc_callback(procmod_Callback_Params* params)
{
  double elapsedtime;
...
static int callbacktests_init(void)
{
  ...
  processbegin_handle = procmod_register_callback(PROCMOD_PROCESSBEGIN_CB, &callbacktests_loadmainmodule_callback, NULL);
  removeproc_handle = procmod_register_callback(PROCMOD_REMOVEPROC_CB, &callbacktests_removeproc_callback, NULL);
